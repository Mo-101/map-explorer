...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    for (const feature of features) {
      ...
      if (!isInAfrica(lat, lon)) { skippedNonAfrica++; continue; }
    ...
    return new Response(JSON.stringify({ status: "ok", run_id: runId,
      total_features: features.length, upserted: upsertCount, skipped_non_africa: skippedNonAfrica }),
      { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (e: any) {
