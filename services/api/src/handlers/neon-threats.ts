...
interface ThreatCluster {
  cluster_id: string; type: string; severity: string; title: string; description: string;
  center_lat: number; center_lng: number; threat_count: number; threats: any[];
  hull: [number, number][]; max_intensity: number; min_intensity: number; avg_intensity: number;
  sources: string[]; timestamp: string;
}
...
function clusterThreats(threats: RawThreat[]): ThreatCluster[] {
  ...
  for (const [, members] of groups) {
    ...
    const highestSev = members.reduce((best, m) =>
      (severityOrder[m.severity] || 0) > (severityOrder[best] || 0) ? m.severity : best, members[0].severity);
    ...
    clusters.push({
      ...
      type, severity: highestSev, title, description,
      center_lat: centerLat, center_lng: centerLng,
      threat_count: members.length,
      threats: members.map(m => ({
        id: String(m.id), type: m.type, threat_type: m.type,
        severity: m.severity, title: m.title, description: m.description,
        timestamp: m.event_at || m.created_at,
        center_lat: m.lat, center_lng: m.lng, latitude: m.lat, longitude: m.lng,
        intensity: m.intensity, detection_details: m.metadata ?? {},
        source_artifact: m.source_artifact, data_source_run_id: m.data_source_run_id,
        forecast_hour: m.forecast_hour, created_at: m.created_at, updated_at: m.updated_at,
      })),
      hull, max_intensity: maxIntensity, min_intensity: minIntensity,
      avg_intensity: Math.round(avgIntensity * 10) / 10,
      sources, timestamp: members[0].event_at || members[0].created_at,
    });
  }
...
function convexHull(points: [number, number][]): [number, number][] {
  ...
  for (let i = 1; i < unique.length; i++) {
    if (unique[i][1] < unique[start][1] || (unique[i][1] === unique[start][1] && unique[i][0] < unique[start][0])) start = i;
  }
  [unique[0], unique[start]] = [unique[start], unique[0]];

  const pivot = unique[0];
  unique.slice(1).sort((a, b) => Math.atan2(a[1] - pivot[1], a[0] - pivot[0]) - Math.atan2(b[1] - pivot[1], b[0] - pivot[0]));

  const stack: [number, number][] = [unique[0], unique[1]];
  for (let i = 2; i < unique.length; i++) {
    while (stack.length > 1 && cross(stack[stack.length - 2], stack[stack.length - 1], unique[i]) <= 0) stack.pop();
    stack.push(unique[i]);
  }
  stack.push(stack[0]);
...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    const rawThreats: RawThreat[] = rows
      ...
      .map((r: any) => ({
        id: String(r.id), type: r.type, severity: r.severity, title: r.title, description: r.description,
    ...
    if (!clustered) {
      const threats = rawThreats.map(r => ({
        id: r.id, type: r.type, threat_type: r.type, severity: r.severity,
        title: r.title, description: r.description, timestamp: r.event_at || r.created_at,
        center_lat: r.lat, center_lng: r.lng, latitude: r.lat, longitude: r.lng,
      ...
      return new Response(JSON.stringify({ threats, count: threats.length, generated_at: new Date().toISOString() }),
    ...
    const activeRuns = await sql`
      ...
      WHERE is_active = true AND data_source_run_id IS NOT NULL;` as any[];
    const bySource = await sql`
      SELECT source, COUNT(*)::int AS count FROM hazard_alerts
      WHERE is_active = true GROUP BY source;` as any[];
    ...
    return new Response(JSON.stringify({ error: e?.message || String(e), threats: [], clusters: [], count: 0 }),
      { status: 503, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  }
}
