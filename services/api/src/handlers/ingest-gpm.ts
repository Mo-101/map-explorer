...
const THRESHOLDS = {
  rain_24h_high: 100, rain_24h_extreme: 200,
  rain_72h_high: 200, rain_72h_extreme: 400,
};
...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (!neonUrl) return new Response(JSON.stringify({ error: "missing NEON_DATABASE_URL", hazards_found: 0 }),
  ...
  try {
    ...
    const existing = await sql`
      ...
      WHERE source = 'gpm_imerg' AND data_source_run_id = ${runId};` as any[];
    if (existing[0]?.count > 0) {
      return new Response(JSON.stringify({ status: "completed_cleanup", reason: "run already ingested, stale alerts cleaned", run_id: runId }),
        { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    const runArtifact = { source: "nasa_gpm_imerg", run_id: runId, product: "near_real_time",
      resolution: "0.1deg_30min", monitoring_points: MONITORING_POINTS.length,
    ...
    await fetchBatch(MONITORING_POINTS, async (pt) => {
      try {
        const pastDays = 3;
        const url = `https://api.open-meteo.com/v1/forecast?latitude=${pt.lat}&longitude=${pt.lon}&hourly=precipitation&past_days=${pastDays}&forecast_days=1&timezone=UTC`;
        ...
        const accum24h = last24h.reduce((s, v) => s + v, 0);
        const last72h = precipValues.slice(Math.max(0, totalHours - 72));
        const accum72h = last72h.reduce((s, v) => s + v, 0);

        precipGrid.push({ lat: pt.lat, lon: pt.lon, name: pt.name,
          accum24h: Math.round(accum24h * 10) / 10,
          accum72h: Math.round(accum72h * 10) / 10,
          hourlyData: last24h });

        const pointArtifact = { location: pt.name, lat: pt.lat, lon: pt.lon,
          accum_24h_mm: Math.round(accum24h * 10) / 10,
          accum_72h_mm: Math.round(accum72h * 10) / 10,
          hourly_samples: totalHours, variable: "precipitation" };
      ...
      } catch (ptErr) { console.log(`GPM error for ${pt.name}:`, ptErr); }
    }, 5);
    ...
    return new Response(JSON.stringify({
      status: "completed", source: "gpm_imerg_via_open_meteo", run_id: runId,
      points_scanned: MONITORING_POINTS.length,
      hazards_detected: allHazards.length, hazards_upserted: upserted,
      precip_grid_points: precipGrid.length, precip_grid: precipGrid,
      thresholds: THRESHOLDS, run_artifact: runArtifact, timestamp: new Date().toISOString(),
    }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (e: any) {
