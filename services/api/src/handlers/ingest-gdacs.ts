...
const TYPE_MAP: Record<string, string> = { EQ: "earthquake", TC: "cyclone", FL: "flood", VO: "volcano", DR: "drought" };
const SEVERITY_MAP: Record<string, string> = { Red: "extreme", Orange: "high", Green: "medium" };
...
function enrichGdacsMetadata(props: any, eventType: string, alertLevel: string) {
  ...
  if (eventType === "drought") gdacs.drought_score = props.alertscore ?? null;
  ...
  if (country) {
    ...
    if (vuln !== undefined) { gdacs.inform_vulnerability = vuln; gdacs.country = country; }
  }
  return gdacs;
}
...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    const resp = await fetch(gdacsUrl, { headers: { Accept: "application/json" } });
    ...
    for (const feature of features) {
      ...
      const metadata = { alertscore: props.alertscore, country: props.country, fromdate: props.fromdate, todate: props.todate, gdacs };
      ...
      await sql`
        ...
        ON CONFLICT (source, external_id) DO UPDATE SET
          ...
          lat = EXCLUDED.lat, lng = EXCLUDED.lng, is_active = true,
          data_source_run_id = EXCLUDED.data_source_run_id, last_seen_at = NOW(),
          metadata = EXCLUDED.metadata, updated_at = NOW()`;
      upsertCount++;
    }

    return new Response(JSON.stringify({ status: "ok", run_id: runId, total_events: features.length, africa_upserted: upsertCount }),
      { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (e: any) {
