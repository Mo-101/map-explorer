...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    try {
      ...
      const resp = await fetch(apiUrl, { headers: { Accept: "application/json", "User-Agent": "AfroStorm/1.0 (health monitoring)" } });
      if (resp.ok) {
    ...
    } catch (e) { console.warn("[ingest-who-don] JSON API failed:", e); }
    ...
    if (items.length === 0) {
      ...
      for (const rssUrl of rssUrls) {
        try {
          const resp = await fetch(rssUrl, { headers: { Accept: "application/xml, text/xml, */*", "User-Agent": "AfroStorm/1.0" } });
    ...
    if (items.length === 0) {
      return new Response(JSON.stringify({ status: "no_data", run_id: runId, message: "Could not fetch WHO DON from any endpoint" }),
        { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    for (const item of items) {
      ...
      await sql`
        ...
        ON CONFLICT (source, external_id) DO UPDATE SET
          title = EXCLUDED.title, description = EXCLUDED.description, is_active = true,
          data_source_run_id = EXCLUDED.data_source_run_id, last_seen_at = NOW(),
          metadata = EXCLUDED.metadata, updated_at = NOW()`;
      upsertCount++;
    }

    return new Response(JSON.stringify({ status: "ok", run_id: runId, api_source: apiSource,
