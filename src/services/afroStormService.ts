...
export interface AfroStormThreat {
  id: string;
  threat_type: 'cyclone' | 'cholera' | 'lassa' | 'meningitis' | 'flood' | 'drought';
  ...
  try {
    ...
    if (lastBackendStatusLogged !== backendAvailable) {
      if (backendAvailable) {
        ...
        console.log('[AFRO STORM] Backend unavailable, using mock data');
      }
    ...
    if (lastBackendStatusLogged !== backendAvailable) {
      console.log('[AFRO STORM] Backend offline, using mock data');
      lastBackendStatusLogged = backendAvailable;
    }
  } finally {
 ...
 * In production, this fetches from the Python FastAPI backend
 */
function generateMockThreats(): AfroStormResponse {
  const now = new Date();
...
export async function getAfroStormThreats(): Promise<AfroStormResponse> {
  ...
  // If no backend configured, avoid hitting non-existent routes and use mock in dev.
  if (!AFRO_STORM_CONFIG.primaryBackend) {
    if (IS_DEV && AFRO_STORM_CONFIG.enableMockData) {
      const data = generateMockThreats();
  ...
  // One quiet health check (throttled) to avoid ERR_CONNECTION_REFUSED spam.
  ...
  // Check circuit breaker to prevent hammering dead backend
  if (isCircuitOpen()) {
    console.log('[AFRO STORM] Circuit breaker open - backing off');

    // Use cached data if available
    if (cache.data && (now - cache.timestamp) < CACHE_TTL * 2) {
      console.log('[AFRO STORM] Using cached data (circuit open)');
      return cache.data;
    }

    // In dev, use mock data; in prod, throw error
    if (IS_DEV) {
      console.log('[AFRO STORM] Circuit open - using mock data');
      const data = generateMockThreats();
      cache = { data, timestamp: now };
      return data;
    }
  ...
  try {
    ...
    if (IS_DEV) {
      // Generate mock data only in development to avoid silent failures in production
      const data = generateMockThreats();
      cache = { data, timestamp: now };
      return data;
    }

    // In production, bubble up so UI can show "backend offline" instead of silently mocking
    throw error;
  }
}
...
export function formatThreatDescription(threat: AfroStormThreat): string {
  ...
  switch (threat.threat_type) {
    ...
    case 'lassa':
      return `Predicted ${details.predicted_cases || 'N/A'} cases. ${details.season_factor || 'Seasonal risk'}. Confidence: ${Math.round(threat.confidence * 100)}%`;
    ...
    default:
      return `Health threat detected. Lead time: ${threat.lead_time_days} days.`;
  }
}
...
export function getBackendStatus() {
  return {
    ...
    mockDataEnabled: AFRO_STORM_CONFIG.enableMockData
  };
}
